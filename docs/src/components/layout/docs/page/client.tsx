...
export function PageTOCPopoverTrigger({
  ...
  return (
    <CollapsibleTrigger
      className={cn(
        "flex w-full h-10 items-center text-sm text-text-low gap-2.5 px-4 py-2.5 text-start focus-visible:outline-none [&_svg]:size-4 md:px-6",
        className,
      )}
    ...
    >
      <ProgressCircle
        ...
        max={1}
        className={cn("shrink-0 text-brand-base")}
      />
      <span className="grid flex-1 *:my-auto *:row-start-1 *:col-start-1">
        <span
          className={cn(
            "truncate transition-[opacity,translate,color] font-mono tracking-tight",
            open && "text-text-high",
        ...
        <span
          className={cn(
            "truncate transition-[opacity,translate] font-mono tracking-tight",
            !showItem && "opacity-0 translate-y-full pointer-events-none",
          )}
        >
