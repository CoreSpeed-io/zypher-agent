...
export const ReadFileTool = defineTool({
  ...
  parameters: z.object({
    relativePath: z.string().describe('The path of the file to read, relative to the workspace root.'),
    startLineOneIndexed: z.number().describe('The one-indexed line number to start reading from (inclusive).'),
    endLineOneIndexedInclusive: z.number().describe('The one-indexed line number to end reading at (inclusive).'),
    shouldReadEntireFile: z.boolean().describe('Whether to read the entire file. Defaults to false.'),
  ...
  execute: async ({ relativePath, startLineOneIndexed, endLineOneIndexedInclusive, shouldReadEntireFile }) => {
    try {
